...
## Description
Update application (transmit changed parameters other than unchanged parameters)
...
|**grantTypes**|String|False| |Supported OAuth types: <br/> authorization_code: OAuth2 authorization code mode <br/> implicit: OAuth2 implicit authorization mode <br/> refresh_token: enable refresh tokens <br/><br/> support the following values:<br/>  (1) authorization_code<br/> (2) authorization_code, refresh_token<br/> (3) authorization_code,implicit<br/> (4) authorization_code,implicit,refresh_token<br/> (5) implicit<br/><br/>        note: if grantTypes has designated refresh_token, then application will be able to use Refresh Token; If refresh_token is not designated when creating the application, then the application cannot use Refresh Token; application can call "Update Application" interface to change grantTypes setting at any time|
