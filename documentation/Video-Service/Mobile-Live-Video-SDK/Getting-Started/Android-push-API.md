...
// Set the push video rate, the three parameters are the initial code rate, the highest code rate, and the lowest bit rate.
...
//Set start preview to use front or rear camera
...
```
// Set Info callback to receive notification information
...
// Set the error callback. After receiving the callback, a serious error usually occurs, such as network disconnection.
// The SDK will stop pushing traffic internally, and the APP can add retry logic here based on the callback type and requirements.
...
```
// Set the rotation angle of the screen to support 0, 90, 180, 270
mStreamer.setRotateDegrees(0);
//Set start preview to use front or rear camera
mStreamer.setCameraFacing(CameraCapture.FACING_FRONT);
// Switch flash, true is on, false is off
mStreamer.toggleTorch(true);
// Set the beauty filter, please refer to the special description for the specific description of the beauty filter.
...
// Generally you can turn on the camera preview in onResume
        mStreamer.startCameraPreview();
//If the DummyAudio module is cut in onPause, it can be restored here.
    ...
    public void onPause() {
        super.onPause();
        mStreamer.onPause();
        // Generally stop camera acquisition here
        mStreamer.stopCameraPreview();
// If you want the app to cut the background and stop recording the sound of the anchor, you can switch to
//DummyAudio acquisition, this module will generate mute data instead of mic acquisition module, and release the occupied mic resources.
