...
The Live Video Service supports the live snapshot and video streaming is snapshotted as per the time interval setting and stored in Object Storage Service in formats of jpg and png. To have the function, please enable Object Storage Service of JD Cloud and create corresponding Bucket space in North China (now, Live Video is supported in North China only). After Live Video is started, snapshot function will be automatically started. After images are produced, customers can view and download image files from the storage path.
```
