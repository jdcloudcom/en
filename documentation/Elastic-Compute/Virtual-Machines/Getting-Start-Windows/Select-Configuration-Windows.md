...
* Region and Availability Zone:
	...
	Currently, it covers four domestic regions with multiple availability zones including **cn-north-1 (three availability zones), cn-south-1 (single availability zone), cn-east-1 (single availability zone), cn-east-2 (three availability zones)**. These regions are completely isolated from each other to ensure maximum stability and fault tolerance between each two. Please refer to [Region and Availability Zone] for detailed regional information.  
	...
	* As for the deployment area of ​​other JD Cloud & AI products and the communication requirements of the instance, by default, cross-region mutual intranet access of the instances cannot be realized and access to other cloud products, such as the cloud database and cloud cache, cannot be realized neither. The instance only supports associating the EIP and Load Balancer under the same region.
...
* Storage:
    ...
    JD Cloud & AI provides cloud disk and local disk. A local disk is a storage device on a physical machine where the cloud server is located. It can obtain a lower delay, but there is a risk of single point data loss. The Cloud Disk adopts a multi-backup and distributed storage mode with high data reliability. Cloud Disk Service provides various types of Cloud Disks to be taken by instances as system and data disks. See [Cloud Disk Service Type] for details.
    
    * System Disk: It supports local disks and Cloud Disk; specifically, the local disk has free-of-charge 40GB, which, moreover, cannot be changed. Cloud Disk supports 40GB~500GB.
    * Data Disk: If the system disk is a local disk, 8 data disks can be attached. If the system disk is Cloud Disk, 7 data disks can be attached.
...
* Networking:
    * VPC and Subnet: You need to create a VPC and a subnet first. Please see [VPC](http://docs.jdcloud.com/virtual-private-cloud/product-overview) and [Subnet](http://docs.jdcloud.com/virtual-private-cloud/subnet-features) for details.
    ...
    * Security Group: You can create a security group that meets the IAM requirement under each VPC. If not, it supports to associate one from the three security group modules created automatically by the system. Please see [Security Group Overview](http://docs.jdcloud.com/virtual-private-cloud/security-group-features) for details.
...
[Region and Availability Zone](http://docs.jdcloud.com/virtual-machines/regions-and-availabilityzones)
...
[Cloud Disk Type](http://docs.jdcloud.com/cloud-disk-service/specifications)

[VPC](http://docs.jdcloud.com/virtual-private-cloud/product-overview)

[Subnet](http://docs.jdcloud.com/virtual-private-cloud/subnet-features)

[Security Group Overview](http://docs.jdcloud.com/virtual-private-cloud/security-group-features)
