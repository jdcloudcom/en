...
|  Disk Read and Write Traffic(host)     |  Average disk read/write traffic per second, unit: Bps      |   Write/read data aggregate per second by all disks of the current instance. ‘host’ means that the indicator shall be reported by a host machine.              |

## Monitoring Data Description
* Monitoring data collection period is 10s, with the minimum statistical period of 1min;
