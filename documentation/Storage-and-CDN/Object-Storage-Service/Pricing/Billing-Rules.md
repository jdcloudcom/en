...
This paper mainly introduces the components and billing methods of OSS service charge of JD Cloud & AI object storage, and you can learn more about OSS service charge collection through this paper.
...
All regions of JD Cloud & AI object storage service adopt the quantity-based pay-as-you-go billing mode, which pushes the bill on a daily basis according to the actual consumption of user and collects charge for the previous day as per the bill per day.
...
Traffic means the cumulative value of network bandwidth traffic you use. Specific measurement items include downlink traffic of Internet, downlink traffic of Intranet, uplink traffic of Internet, uplink traffic of Intranet, CDN back-to-origin downlink traffic and data synchronization traffic. See [Pricing](./Price-Overview.md) for detailed price.

|Measurement Item|Measurement Method|Billing Method|
|---|----|----|
|Uplink Traffic of Intranet|Uplink traffic generated by uploading data from internal service such as Virtual Machines to object storage service via JD Cloud & AI Intranet. |Free|
|Downlink Traffic of Intranet|Downlink traffic generated by downloading data from object storage service to internal service such as Virtual Machines via JD Cloud & AI Intranet. |Free|
|Uplink Traffic of Internet|Uplink traffic generated by uploading data from local end to JD Cloud & AI Object Storage Service via public network |Free|
|Downlink Traffic of Internet|Downlink traffic generated by downloading data from JD Cloud & AI object storage service to local end via public network. |The traffic rate will be charged per day, accumulatively. <br>Every day’s accumulative downlink traffic of Internet (GB)\*Unit price per GB. For example, in certain Bucket in Beijing Region, the downlink traffic of Internet has generated 100GB accumulatively, so the cost of downlink traffic of Internet is 0.5\*100 (GB) = RMB 50|
|CDN Back-to-origin Downlink Traffic|Back-to-origin downlink traffic generated by downloading OSS data via CDN service layer. |CDN back-to-origin downlink traffic (GB) accumulated everyday\*Unit price per GB|
|Data Synchronization Traffic|Traffic generated by synchronizing data from source Bucket to target Bucket via data synchronization function. |Every day’s accumulative data synchronization traffic (GB)\*Unit price per GB.|


### Cost of Number of Requests
<div id="user-content-5"></div>

All your OSS operations are ultimately calls to the OSS APIs, whether through the Console, life cycle management, data synchronization or other tools. APIs call cost will be billed by the times you call OSS APIs. See [Pricing](./Price-Overview.md) for cost of times of write operations and read operations.

|Measurement Item|Measurement Method|Billing Method|
|---|----|----|
|Times of Call OSS APIs|Request Times of Call OSS APIs |The cost of calling OSS APIs is settled per day. Formula for calculating the cost of request times: Unit price per 10,000 calls \* Actual times of calls per day /10000. |
