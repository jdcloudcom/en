...
2. Download the backup unzipping tool, [Click to Download](http://jddb-common-public.oss.cn-north-1.jcloudcs.com/general_mysql_backup_extract_tool.zip) and unzip. The tool name is `mysql_backup_extract.py`, and the using instances are as follows.
...
4. Unzip the backup data, and the unzipped files will be saved in tmp_snapshot, a sub-directory of the current directory, assuming that the current directory is $HOME.
    ...
    ```
    ./mysql_backup_extract.py -v 5.7 -f <Customized backup file name>.xbstream
