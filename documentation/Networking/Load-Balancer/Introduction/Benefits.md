...
|A variety of service types| The JD Cloud Application Load Balancer can register the virtual machine and the container instance to support the business backend distribution service and has a variety of service types |	Supports backend distribution service with a physical server|
...
|Easy operation| The JD Cloud Application Load Balancer can realize modular decoupling and highly multiplex of backend service, virtual server group and availability group and simplify configuration. For example, listeners of different protocols (providing business of HTTP and HTTPS services at the same time) can multiplex the same backend service |	The configuration is fuzzy, relatively|
